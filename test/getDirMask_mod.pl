...
#		print "DEBUG ===== $entry\n";
		my ($endstring) = index($entry,".dat")+length(".dat");
		my ($diff) = length($entry)-$endstring;
...
#			print "$key has mask: $mask \n";
		    }else {
#			print "already found: ",$entry," (",$key,") ";
			my ($diff) = length($entry)-$endstring;
#			print "delta=",$diff,"\n";
			if ($diff>0) { 
#			    print "skipped because same as ",$key," <-- ",$entry,"\n";
			}else{
			    $run{$key}[0] += $mask;      # switch on a new bit
#			    print " not skipped ",$entry," -> ",$mask,"\n";
			}		    
		    }
		}else {
