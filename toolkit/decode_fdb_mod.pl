...
sub getDataMigrCommand {
# Check input
    my($mode) =  $_[0];
    my($setupFile) = $_[1];
    my $key = "^GENERAL::RAWDATAMIGR_COMMAND";
...
sub getDataMigrTimeout {
# Check input
    my($mode) =  $_[0];
    my($setupFile) = $_[1];
    my $key = "^GENERAL::RAWDATAMIGR_TIMEOUT";
    my $n = getOptionHost($mode,$setupFile,$key);
    $mode eq "-verbose" && print "$key $n\n";
    return ($n);
...
sub getCASTORstagehost {
# Check input
    my($mode) =  $_[0];
    my($setupFile) = $_[1];
    my $key = "^CASTOR::STAGE_HOST";
    my $n = getOptionHost($mode,$setupFile,$key);
    $mode eq "-verbose" && print "$key $n\n";
    return ($n);
...
sub getCASTORstagepool {
# Check input
    my($mode) =  $_[0];
    my($setupFile) = $_[1];
    my $key = "^CASTOR::STAGE_POOL";
    my $n = getOptionHost($mode,$setupFile,$key);
    $mode eq "-verbose" && print "$key $n\n";
    return ($n);
...
sub getCASTORsvcclass {
# Check input
    my($mode) =  $_[0];
    my($setupFile) = $_[1];
    my $key = "^CASTOR::SVCCLASS";
    my $n = getOptionHost($mode,$setupFile,$key);
    $mode eq "-verbose" && print "$key $n\n";
    return ($n);
...
# Check input
    my($mode) =  $_[0];
    my($setupFile) = $_[1];
    my $key = "^CASTOR::RFIO_USE_CASTOR_V2";
    my $n = getOptionHost($mode,$setupFile,$key);
    if ($mode eq "-verbose") {
	print "$key $n\n";
    }
